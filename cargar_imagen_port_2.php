...
if (!isset($_FILES['portfolioImage2'])) {
    echo "No se recibió ninguna imagen";
    exit;
}

$file = $_FILES['portfolioImage2'];
...
if (move_uploaded_file($tmp, $destino)) {
    ...
    $query = $conn->prepare("UPDATE imagenes SET Digital2 = ? WHERE Correo = ?;");
    $query->bind_param("ss", $originalName, $usuario);
    $query->execute();
    $_SESSION["usuario_digital2"] = $originalName;
