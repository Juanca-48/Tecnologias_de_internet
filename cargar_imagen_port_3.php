...
if (!isset($_FILES['portfolioImage3'])) {
    echo "No se recibió ninguna imagen";
    exit;
}

$file = $_FILES['portfolioImage3'];
...
if (move_uploaded_file($tmp, $destino)) {
    ...
    $query = $conn->prepare("UPDATE imagenes SET Digital3 = ? WHERE Correo = ?;");
    $query->bind_param("ss", $originalName, $usuario);
    $query->execute();
    $_SESSION["usuario_digital3"] = $originalName;
