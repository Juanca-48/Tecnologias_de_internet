...
if (!isset($_FILES['comisionImage'])) {
    echo "No se recibió ninguna imagen";
    exit;
}

$file = $_FILES['comisionImage'];
...
if (move_uploaded_file($tmp, $destino)) {
    ...
    $query = $conn->prepare("UPDATE imagenes SET Comision = ? WHERE Correo = ?;");
    $query->bind_param("ss", $originalName, $usuario);
    $query->execute();
    $_SESSION["usuario_comision"] = $originalName;
