...
if (!isset($_FILES['portfolioImage1'])) {
    echo "No se recibió ninguna imagen";
    exit;
}

$file = $_FILES['portfolioImage1'];
...
if (move_uploaded_file($tmp, $destino)) {
    ...
    $query = $conn->prepare("UPDATE imagenes SET Digital1 = ? WHERE Correo = ?;");
    $query->bind_param("ss", $originalName, $usuario);
    $query->execute();
    $_SESSION["usuario_digital1"] = $originalName;
